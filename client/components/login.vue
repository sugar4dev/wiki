  ...
  <v-app>
    <div class="login" :style="`background-image: url(` + bgUrl + `);`">
      <div class="login-sd">
        ...
        <v-alert class="mb-0" v-model="errorShown" transition="slide-y-reverse-transition" color="red darken-2" tile dark dense icon="mdi-alert">
        ...
        <template v-if="screen === `login` && strategies.length > 1">
          ...
          <div class="login-list">
            <v-list class="elevation-1 radius-7" nav light>
              <v-list-item-group v-model="selectedStrategyKey">
                <v-list-item v-for="(stg, idx) of filteredStrategies" :key="stg.key" :value="stg.key" :color="stg.strategy.color">
                  <v-avatar class="mr-3" tile size="24" v-html="stg.strategy.icon"></v-avatar><span class="text-none">{{stg.displayName}}</span>
                </v-list-item>
              </v-list-item-group>
            </v-list>
          </div>
        ...
        <template v-if="screen === `login` && selectedStrategy.strategy.useForm">
          ...
          <div class="login-form">
            <v-text-field solo flat prepend-inner-icon="mdi-clipboard-account" background-color="white" color="blue darken-2" hide-details ref="iptEmail" v-model="username" :placeholder="isUsernameEmail ? $t(`auth:fields.email`) : $t(`auth:fields.username`)" :type="isUsernameEmail ? `email` : `text`" :autocomplete="isUsernameEmail ? `email` : `username`" light></v-text-field>
            <v-text-field class="mt-2" solo flat prepend-inner-icon="mdi-form-textbox-password" background-color="white" color="blue darken-2" hide-details ref="iptPassword" v-model="password" :append-icon="hidePassword ? 'mdi-eye-off' : 'mdi-eye'" @click:append="() => (hidePassword = !hidePassword)" :type="hidePassword ? 'password' : 'text'" :placeholder="$t('auth:fields.password')" autocomplete="current-password" @keyup.enter="login" light></v-text-field>
            <v-btn class="mt-2 text-none" width="100%" large color="blue darken-2" dark @click="login" :loading="isLoading">{{ $t('auth:actions.login') }}</v-btn>
            <div class="text-center mt-5">
              <v-btn class="text-none" text rounded color="grey darken-3" @click.stop.prevent="forgotPassword" href="#forgot">
                <div class="caption">{{ $t('auth:forgotPasswordLink') }}</div>
              </v-btn>
              <v-btn class="text-none" v-if="selectedStrategyKey === `local` && selectedStrategy.selfRegistration" color="indigo darken-2" text rounded href="/register">
        ...
        <template v-if="screen === `forgot`">
          ...
          <div class="login-form">
            <v-text-field solo flat prepend-inner-icon="mdi-clipboard-account" background-color="white" color="blue darken-2" hide-details ref="iptForgotPwdEmail" v-model="username" :placeholder="$t(`auth:fields.email`)" type="email" autocomplete="email" light></v-text-field>
            <v-btn class="mt-2 text-none" width="100%" large color="blue darken-2" dark @click="forgotPasswordSubmit" :loading="isLoading">{{ $t('auth:sendResetPassword') }}</v-btn>
            <div class="text-center mt-5">
              <v-btn class="text-none" text rounded color="grey darken-3" @click.stop.prevent="screen = `login`" href="#forgot">
        ...
        <template v-if="screen === `changePwd`">
          ...
          <div class="login-form">
            <v-text-field class="mt-2" type="password" solo flat prepend-inner-icon="mdi-form-textbox-password" background-color="white" color="blue darken-2" hide-details ref="iptNewPassword" v-model="newPassword" :placeholder="$t(`auth:changePwd.newPasswordPlaceholder`)" autocomplete="new-password" light>
              <password-strength slot="progress" v-model="newPassword"></password-strength>
            </v-text-field>
            <v-text-field class="mt-2" type="password" solo flat prepend-inner-icon="mdi-form-textbox-password" background-color="white" color="blue darken-2" hide-details v-model="newPasswordVerify" :placeholder="$t(`auth:changePwd.newPasswordVerifyPlaceholder`)" autocomplete="new-password" @keyup.enter="changePassword" light></v-text-field>
            <v-btn class="mt-2 text-none" width="100%" large color="blue darken-2" dark @click="changePassword" :loading="isLoading">{{ $t('auth:changePwd.proceed') }}</v-btn>
          </div>
        </template>
      </div>
    </div>
    <v-dialog v-model="isTFAShown" max-width="500" persistent>
      <v-card>
        <div class="login-tfa text-center pa-5 grey--text text--darken-3"><img src="_assets/svg/icon-pin-pad.svg">
          ...
          <v-text-field class="login-tfa-field mt-2" solo flat background-color="white" color="blue darken-2" hide-details ref="iptTFA" v-model="securityCode" :placeholder="$t('auth:tfa.placeholder')" autocomplete="one-time-code" @keyup.enter="verifySecurityCode(false)" light></v-text-field>
          <v-btn class="mt-2 text-none" width="100%" large color="blue darken-2" dark @click="verifySecurityCode(false)" :loading="isLoading">{{ $t('auth:tfa.verifyToken') }}</v-btn>
        </div>
      </v-card>
    </v-dialog>
    <v-dialog v-model="isTFASetupShown" max-width="600" persistent>
      <v-card>
        <div class="login-tfa text-center pa-5 grey--text text--darken-3">
          ...
          <div class="caption">(<a href="https://authy.com/" target="_blank" noopener>Authy</a>, <a href="https://support.google.com/accounts/answer/1066447" target="_blank" noopener>Google Authenticator</a>, <a href="https://www.microsoft.com/en-us/account/authenticator" target="_blank" noopener>Microsoft Authenticator</a>, etc.)</div>
          ...
          <v-text-field class="login-tfa-field mt-2" solo flat background-color="white" color="blue darken-2" hide-details ref="iptTFASetup" v-model="securityCode" :placeholder="$t('auth:tfa.placeholder')" autocomplete="one-time-code" @keyup.enter="verifySecurityCode(true)" light></v-text-field>
          <v-btn class="mt-2 text-none" width="100%" large color="blue darken-2" dark @click="verifySecurityCode(true)" :loading="isLoading">{{ $t('auth:tfa.verifyToken') }}</v-btn>
        </div>
      </v-card>
    </v-dialog>
