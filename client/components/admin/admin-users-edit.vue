<template>  
  <v-container fluid grid-list-lg>
    <v-layout row wrap>
      <v-flex xs12>
        <div class="admin-header"><img class="animated fadeInUp" src="/_assets/svg/icon-male-user.svg" :alt="$t(`admin:users.edit`)" style="width: 80px;">
          <div class="admin-header-title">
            <div class="headline blue--text text--darken-2 animated fadeInLeft">{{$t('admin:users.edit')}}</div>
            <div class="subtitle-1 grey--text animated fadeInLeft wait-p2s">{{user.name}}</div>
          </div>
          <v-spacer></v-spacer>
          <i18next class="pr-4 caption grey--text animated fadeInDown" path="admin:users.id" tag="div"><strong place="id">{{user.id}}</strong></i18next>
          <template v-if="user.isActive">
            <status-indicator class="mr-3" positive pulse></status-indicator>
            <div class="caption green--text">{{$t('admin:users.active')}}</div>
          </template>
          <template v-else>
            <status-indicator class="mr-3" negative pulse></status-indicator>
            <div class="caption red--text">{{$t('admin:users.inactive')}}</div>
          </template>
          <template v-if="user.isVerified">
            <status-indicator class="mr-3 ml-4" active pulse></status-indicator>
            <div class="caption blue--text">{{$t('admin:users.verified')}}</div>
          </template>
          <template v-else>
            <status-indicator class="mr-3 ml-4" intermediary pulse></status-indicator>
            <div class="caption deep-orange--text">{{$t('admin:users.unverified')}}</div>
          </template>
          <v-spacer></v-spacer>
          <v-btn class="ml-3 animated fadeInDown wait-p3s" color="grey" icon outlined to="/users">
            <v-icon>mdi-arrow-left</v-icon>
          </v-btn>
          <v-menu offset-y origin="top right">
            <template v-slot:activator="{ on }">
              <v-btn class="ml-3 animated fadeInDown wait-p2s" color="black" v-on="on" depressed dark><span>Actions</span>
            ...
            <v-list dense nav>
              ...
              <v-list-item @click="verifyUser" :disabled="user.isVerified">
                ...
                <v-list-item-title>Set as Verified</v-list-item-title>
              </v-list-item>
          ...
          <v-btn class="ml-3 animated fadeInDown" color="primary" large depressed @click="updateUser">
            <v-icon left>mdi-check</v-icon><span>{{$t('admin:users.updateUser')}}</span>
          </v-btn>
        </div>
      </v-flex>
      <v-flex xs6>
        <v-card class="animated fadeInUp">
          <v-toolbar color="primary" dense dark flat>
            <v-icon class="mr-2">mdi-information-variant</v-icon><span>{{$t('admin:users.basicInfo')}}</span>
          </v-toolbar>
          <v-list class="py-0" two-line dense>
            <v-list-item>
              ...
              <v-list-item-content>
                <v-list-item-title>{{$t('admin:users.email')}}</v-list-item-title>
                <v-list-item-subtitle>{{ user.email }}</v-list-item-subtitle>
              </v-list-item-content>
              <v-list-item-action v-if="!user.isSystem && user.providerKey === `local`">
                <v-menu v-model="editPop.email" :close-on-content-click="false" min-width="350" left>
                  <template v-slot:activator="{ on }">
                  ...
                  <v-card>
                    <v-text-field ref="iptEmail" v-model="user.email" :label="$t(`admin:users.email`)" solo hide-details append-icon="mdi-check" @click:append="editPop.email = false" @keydown.13="editPop.email = false" @keydown.esc="editPop.email = false"></v-text-field>
                  </v-card>
                </v-menu>
              </v-list-item-action>
            </v-list-item>
            ...
            <v-list-item>
              ...
              <v-list-item-content>
                <v-list-item-title>{{$t('admin:users.displayName')}}</v-list-item-title>
              ...
              <v-list-item-action>
                <v-menu v-model="editPop.name" :close-on-content-click="false" min-width="350" left>
                  ...
                  <v-card>
                    <v-text-field ref="iptDisplayName" v-model="user.name" :label="$t(`admin:users.displayName`)" solo hide-details append-icon="mdi-check" @click:append="editPop.name = false" @keydown.13="editPop.name = false" @keydown.esc="editPop.name = false"></v-text-field>
                  </v-card>
                </v-menu>
              </v-list-item-action>
            </v-list-item>
        ...
        <v-card class="mt-3 animated fadeInUp wait-p2s" v-if="!user.isSystem">
          <v-toolbar color="primary" dense dark flat>
            <v-icon class="mr-2">mdi-lock-outline</v-icon><span>{{$t('admin:users.authentication')}}</span>
          </v-toolbar>
          <v-list class="py-0" two-line dense>
            <v-list-item>
              <v-list-item-avatar size="32">
              ...
              <v-list-item-content>
                <v-list-item-title>{{$t('admin:users.authProvider')}}</v-list-item-title>
                <v-list-item-subtitle>{{ user.providerName }} <em class="caption">({{ user.providerKey }})</em></v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
            <template v-if="user.providerKey === `local`">
              ...
              <v-list-item>
                ...
                <v-list-item-content>
                  <v-list-item-title>{{$t('admin:users.password')}}</v-list-item-title>
                  <v-list-item-subtitle>&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;</v-list-item-subtitle>
                </v-list-item-content>
                <v-list-item-action>
                  <v-menu v-model="editPop.newPassword" :close-on-content-click="false" min-width="350" left>
                    <template v-slot:activator="{ on: menu }">
                      <v-tooltip top>
                        ...
                        </template><span>{{$t('admin:users.changePassword')}}</span>
                      </v-tooltip>
                    </template>
                    <v-card>
                      <v-text-field ref="iptNewPassword" v-model="newPassword" :label="$t(`admin:users.newPassword`)" solo hide-details append-icon="mdi-check" type="password" @click:append="editPop.newPassword = false" @keydown.13="editPop.newPassword = false" @keydown.esc="editPop.newPassword = false"></v-text-field>
                    </v-card>
                  </v-menu>
                </v-list-item-action>
                <v-list-item-action>
                  <v-tooltip top>
                    ...
                    </template><span>Send Password Reset Email</span>
                  </v-tooltip>
                </v-list-item-action>
              </v-list-item>
            </template>
            <template v-if="user.providerIs2FACapable">
              ...
              <v-list-item>
                ...
                <v-list-item-content>
                  <v-list-item-title>{{$t('admin:users.tfa')}}</v-list-item-title>
                  <v-list-item-subtitle class="green--text" v-if="user.tfaIsActive">Active</v-list-item-subtitle>
                  <v-list-item-subtitle class="red--text" v-else>Inactive</v-list-item-subtitle>
                </v-list-item-content>
                <v-list-item-action>
                  <v-tooltip top>
                    <template v-slot:activator="{ on }">
                    ...
                    </template><span>{{$t('admin:users.toggle2FA')}}</span>
                  </v-tooltip>
                </v-list-item-action>
              </v-list-item>
            </template>
            <template v-if="user.providerId">
              ...
              <v-list-item>
                ...
                <v-list-item-content>
                  <v-list-item-title>{{$t('admin:users.authProviderId')}}</v-list-item-title>
                  <v-list-item-subtitle>{{ user.providerId }}</v-list-item-subtitle>
                </v-list-item-content>
              </v-list-item>
            </template>
          </v-list>
        ...
        <v-card class="mt-3 animated fadeInUp wait-p4s">
          <v-toolbar color="primary" dense dark flat>
            <v-icon class="mr-2">mdi-account-group</v-icon><span>{{$t('admin:users.groups')}}</span>
          </v-toolbar>
          <v-list dense>
          ...
          <v-alert class="mx-3" v-if="user.groups.length < 1" outlined color="grey darken-1" icon="mdi-alert">
            <div class="caption">{{$t('admin:users.noGroupAssigned')}}</div>
          </v-alert>
          <v-card-chin v-if="!user.isSystem">
            <v-spacer></v-spacer>
            <v-select ref="iptAssignGroup" :items="groups" v-model="newGroup" :label="$t(`admin:users.selectGroup`)" item-value="id" item-text="name" item-disabled="isSystem" solo flat hide-details @keydown.esc="editPop.assignGroup = false" style="max-width: 300px;" dense></v-select>
            <v-btn class="ml-2 px-4" depressed color="primary" @click="assignGroup" :disabled="newGroup === 0">
              <v-icon left>mdi-clipboard-account-outline</v-icon><span>{{$t('admin:users.groupAssign')}}</span>
            </v-btn>
          </v-card-chin>
          <v-system-bar window :color="$vuetify.theme.dark ? `grey darken-4-l3` : `grey lighten-3`">
            <v-spacer></v-spacer>
            <div class="caption">{{$t('admin:users.groupAssignNotice')}}</div>
          </v-system-bar>
        </v-card>
      </v-flex>
      <v-flex xs6>
        <v-card class="animated fadeInUp wait-p2s">
          <v-toolbar color="primary" dense dark flat>
            <v-icon class="mr-2">mdi-account-badge-outline</v-icon><span>{{$t('admin:users.extendedMetadata')}}</span>
          </v-toolbar>
          <v-list class="py-0" two-line dense>
            <v-list-item>
              <v-list-item-avatar size="32">
              ...
              <v-list-item-content>
                <v-list-item-title>{{$t('admin:users.location')}}</v-list-item-title>
              ...
              <v-list-item-action>
                <v-menu v-model="editPop.location" :close-on-content-click="false" min-width="350" left>
                  ...
                  <v-card>
                    <v-text-field ref="iptLocation" v-model="user.location" :label="$t(`admin:users.location`)" solo hide-details append-icon="mdi-check" @click:append="editPop.location = false" @keydown.13="editPop.location = false" @keydown.esc="editPop.location = false"></v-text-field>
                  </v-card>
                </v-menu>
              </v-list-item-action>
            </v-list-item>
            ...
            <v-list-item>
              ...
              <v-list-item-content>
                <v-list-item-title>{{$t('admin:users.jobTitle')}}</v-list-item-title>
              ...
              <v-list-item-action>
                <v-menu v-model="editPop.jobTitle" :close-on-content-click="false" min-width="350" left>
                  ...
                  <v-card>
                    <v-text-field ref="iptJobTitle" v-model="user.jobTitle" :label="$t(`admin:users.jobTitle`)" solo hide-details append-icon="mdi-check" @click:append="editPop.jobTitle = false" @keydown.13="editPop.jobTitle = false" @keydown.esc="editPop.jobTitle = false"></v-text-field>
                  </v-card>
                </v-menu>
              </v-list-item-action>
            </v-list-item>
            ...
            <v-list-item>
              ...
              <v-list-item-content>
                <v-list-item-title>{{$t('admin:users.timezone')}}</v-list-item-title>
                <v-list-item-subtitle>{{ user.timezone }}</v-list-item-subtitle>
              </v-list-item-content>
              <v-list-item-action>
                <v-menu v-model="editPop.timezone" :close-on-content-click="false" min-width="350" left>
                  <template v-slot:activator="{ on }">
                  ...
                  <v-card>
                    <v-select ref="iptTimezone" :items="timezones" v-model="user.timezone" :label="$t(`admin:users.timezone`)" solo dense hide-details append-icon="mdi-check" @click:append="editPop.timezone = false" @keydown.13="editPop.timezone = false" @keydown.esc="editPop.timezone = false"></v-select>
                  </v-card>
                </v-menu>
              </v-list-item-action>
            </v-list-item>
        ...
        <v-card class="mt-3 animated fadeInUp wait-p6s">
          <v-toolbar color="teal" dense dark flat>
            <v-icon class="mr-2">mdi-file-document-box-multiple-outline</v-icon><span>Content</span>
          </v-toolbar>
          <v-card-text><em class="caption grey--text">Coming soon</em></v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
    <v-dialog v-model="deleteUserDialog" max-width="500">
      <v-card>
        <div class="dialog-header is-red">{{$t('admin:users.deleteConfirmTitle')}}</div>
        <v-card-text class="pt-5">
          <i18next path="admin:users.deleteConfirmText" tag="span"><strong place="username">{{ user.email }}</strong></i18next>
          <div class="mt-3">{{$t('admin:users.deleteConfirmReplaceWarn')}}</div>
          <v-divider class="my-3"></v-divider>
          <div class="d-flex align-center mt-3">
            <v-btn class="text-none" color="primary" depressed @click="deleteSearchUserDialog = true">
              <v-icon left>mdi-clipboard-account</v-icon>Select User...
            </v-btn>
            <div class="caption pl-3"><strong>ID {{deleteReplaceUser.id}}</strong>
              <div class="caption">{{deleteReplaceUser.name}}</div><em>{{deleteReplaceUser.email}}</em>
            </div>
          </div>
        </v-card-text>
        <v-card-chin>
          <v-spacer></v-spacer>
          <v-btn text @click="deleteUserDialog = false">{{$t('common:actions.cancel')}}</v-btn>
          <v-btn color="red" dark @click="deleteUser">{{$t('common:actions.delete')}}</v-btn>
        </v-card-chin>
        <user-search v-model="deleteSearchUserDialog" @select="assignDeleteUser"></user-search>
      </v-card>
    </v-dialog>
  </v-container>
</template>
