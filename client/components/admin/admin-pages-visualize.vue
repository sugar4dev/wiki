<template>  
  <v-container fluid grid-list-lg>
    <v-layout row wrap>
      <v-flex xs12>
        <div class="admin-header"><img class="animated fadeInUp" src="/_assets/svg/icon-venn-diagram.svg" alt="Visualize Pages" style="width: 80px;">
          <div class="admin-header-title">
            <div class="headline blue--text text--darken-2 animated fadeInLeft">Visualize Pages</div>
            <div class="subtitle-1 grey--text animated fadeInLeft wait-p2s">Dendrogram representation of your pages</div>
          </div>
          <v-spacer></v-spacer>
          <v-select class="mx-5 animated fadeInDown wait-p1s" v-if="locales.length > 0" v-model="currentLocale" :items="locales" style="flex: 0 1 120px;" solo dense hide-details item-value="code" item-text="name"></v-select>
          <v-btn-toggle class="animated fadeInDown" v-model="graphMode" color="primary" dense rounded>
            <v-btn class="px-5" value="htree">
              <v-icon left :color="graphMode === `htree` ? `primary` : `grey darken-3`">mdi-sitemap</v-icon><span class="text-none">Hierarchical Tree</span>
            </v-btn>
            <v-btn class="px-5" value="hradial">
              <v-icon left :color="graphMode === `hradial` ? `primary` : `grey darken-3`">mdi-chart-donut-variant</v-icon><span class="text-none">Hierarchical Radial</span>
            </v-btn>
            <v-btn class="px-5" value="rradial">
              <v-icon left :color="graphMode === `rradial` ? `primary` : `grey darken-3`">mdi-blur-radial</v-icon><span class="text-none">Relational Radial</span>
            </v-btn>
          </v-btn-toggle>
        </div>
        <div class="admin-pages-visualize-svg" ref="svgContainer" v-show="pages.length >= 1"></div>
        <v-alert v-if="pages.length < 1" outlined type="warning" style="max-width: 650px; margin: 0 auto;">Looks like there's no data yet to graph!</v-alert>
      </v-flex>
    </v-layout>
  </v-container>
</template>
